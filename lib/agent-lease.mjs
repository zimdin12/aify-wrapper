 ...
 * something else now) or `unknown` (the entry records no identity at all, so it can never be proven
 * ours: it is never stopped, never refuses, and is dropped). An instance of unknown identity takes its
 * attached processes with it -- they may belong to an instance that is still alive.
 *
...
export class AgentLease {
   ...
   *
   * @returns {{decision: "claim"|"refuse"|"nested", reason?: string, live?: object, stopped: object[]}}
   */
  ...
  #state(entry, table) {
    ...
    const state = processes.identify(entry, { alive: Boolean(row), startedAt: row?.startedAtMs });
    return state === "unverified" ? "unknown" : state;
  }
