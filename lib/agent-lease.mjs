 ...
 * is `start`: a guard given a word it cannot read must not end anything. With none, a person at a
 * terminal is explicit (`replace`) and a managed launch, which the service or a host started, is not
 * (`start`).
 */
...
export class AgentLease {
  ...
  claim({ pid, runtime = "", mode = "", intent = "start", inherited = null }) {
    return this.#locked((stillHeld) => {
      ...
      const boundaries = this.#otherAgentsProcesses();
      for (const entry of plan.stop) this.probe.killTree(entry.pid, { table, spare: [pid], boundaries });
      ...
      return { decision: "claim", stopped: plan.stop };
    });
  }
  ...
  /**
   * Give the lease up, if the launcher `pid` still holds it -- UNLESS something it attached is still
   * running. A launcher's exit only signals what it started (hermes kills its delivery loop and leaves the
   * gateway to that loop's teardown; codex signals a shim), and a record deleted first would leave the
   * survivor on no record at all. Kept, it names a dead launcher, so the next claim stops what is left.
   * A probe that fails keeps the record too.
   * @returns {{released: boolean, reason?: "not-the-holder"|"attached-still-running"}}
   */
  release({ pid }) {
    return this.#locked(() => {
      const record = this.read();
      if (!record || record.instance.pid !== pid) return { released: false, reason: "not-the-holder" };
      const attached = (record.instance.attached || []).filter((entry) => entry?.pid !== pid);
      if (attached.length) {
        const times = this.probe.startTimes(attached.map((entry) => entry.pid), { strict: true });
        const running = !times || attached.some((entry) => processes.identify(entry, {
          alive: this.probe.isAlive(entry.pid) && times.has(entry.pid),
          startedAt: times.get(entry.pid),
        }) === "ours");
        if (running) return { released: false, reason: "attached-still-running" };
      }
  ...
  /** Every pid another agent's lease on this host names: subtrees a stop of this agent never enters. */
  #otherAgentsProcesses() {
    const pids = new Set();
    let names = [];
    try {
    ...
    for (const name of names) {
      ...
      try {
        ...
        for (const entry of [instance, ...(instance?.attached || [])]) if (Number.isInteger(entry?.pid)) pids.add(entry.pid);
      } catch {
  ...
  #lockAbandoned(text, ageMs, mtimeMs) {
    ...
    const state = processes.identify({ seenAliveAtMs: Number(holder.atMs) || mtimeMs }, { alive: times.has(pid), startedAt: times.get(pid) });
