 ...
 *
 * `insideSession` is a launch made from inside a RUNNING agent session (a session marker is set), and there
 * only the command's own word replaces anything. The mode cannot carry that decision from inside one: the
 * environment's mode is another session's, so `bin/aify-inherited-session.sh` drops it unless the command
 * names the agent the environment names -- and an aify-env older than the service's unset list hands a
 * worker the marker AND the host's own agent id, which is exactly when it is dropped. The named managed
 * start then read as a person and replaced the live instance (external review, 2026-09-16). A start intent
 * from the ENVIRONMENT is already dropped there, so an explicit value here is one the command carried.
 */
export function startIntent({ explicit, mode, identity, insideSession = false } = {}) {
  const chosen = String(explicit ?? "").trim().toLowerCase();
  if (chosen) return START_INTENTS.includes(chosen) ? chosen : "start";
  if (insideSession) return "start";
