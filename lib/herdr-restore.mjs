...
export class HerdrPaneLedger {
  ...
  #records;
  #unreadable = false;
  //: WHAT THIS WRITER CHANGED since it loaded: record id -> the record, or null for a deletion. `save()`
  //: applies only these, onto what is on disk at that moment. See `save()`.
  #changes = new Map();
  ...
  load() {
    ...
    this.#unreadable = read.unreadable;
    this.#changes = new Map();
    return this;
  }

  /** What is on disk right now, without touching this instance's state. */
  #read() {
    ...
    if (!entries || typeof entries !== "object") return { records, unreadable: true };
  ...
  remember(recordId, entry) {
    ...
    this.#changes.set(recordId, record);
    return this;
  }

  forget(recordId) {
    this.#records.delete(recordId);
    this.#changes.set(recordId, null);
    return this;
  }
   ...
   */
  pruneTo(liveLabels, { listedAt = null } = {}) {
    ...
    for (const [key, record] of [...this.#records.entries()]) {
      ...
      // A LISTING CANNOT JUDGE A RECORD WRITTEN AFTER IT WAS TAKEN. Found by review: the restore pass
      // lists the panes, relaunches wrappers into them, and prunes against that listing -- so a
      // wrapper that claimed its pane in between (a NEW label, and a NEW record) had the record deleted
      // by the very pass that started it, and that pane never came back after the next reboot. A
      // claim renames its pane BEFORE it writes, so a record older than the listing is always judged
      // with its label in view, and a newer one is simply left for the next pass.
      const recordedAt = Date.parse(record?.recordedAt ?? "");
      if (listedAt !== null && Number.isFinite(recordedAt) && recordedAt >= listedAt) continue;
  ...
  save() {
    ...
    // ONLY WHAT THIS WRITER CHANGED, ONTO WHAT IS ON DISK NOW. Found by review: every wrapper that
    // claims a pane loads the whole ledger, adds its record, and writes the whole ledger back -- so two
    // panes launched together, which is exactly what a restore does, could load the same snapshot and
    // the second write erased the first pane's record. Atomic rename stopped half-files, not that.
    //
    // This NARROWS the race to the gap between this read and the rename below, rather than closing
    // it: there is no lock, on purpose, because a lock file left by a crashed wrapper would block every
    // later claim, and a claim must never be able to stall a launch.
    const current = this.#read();
    if (current.unreadable) {
      return { saved: false, why: "the ledger became unreadable before it could be written, so it was left alone" };
    }
    const merged = current.records;
    for (const [key, record] of this.#changes) {
      if (record === null) merged.delete(key);
      else merged.set(key, record);
    }
    const records = Object.fromEntries([...merged.entries()].map(([k, v]) => [k, { ...v }]));
