...
//
// SO THE INSTALLERS ARE ASKED BEFORE PATH, and each platform by its own installer's contract, read
// from Herdr's `distribution/` at v0.9.0. On Windows, `install.ps1` maintains a stable
// `packages/standalone/current` link beside the versioned releases under `HERDR_HOME` (default
// `~/.herdr`) and a visible bin under `HERDR_INSTALL_DIR` (default `%LOCALAPPDATA%/Programs/Herdr/bin`).
// On Linux and macOS, `install.sh` writes one file to `HERDR_INSTALL_DIR` (default `~/.local/bin`) and
// nothing under `~/.herdr` -- searching only the Windows layout refused a working WSL host.
//
 ...
 * which removes the whole class of "talked to a different Herdr". PATH LAST, searched here rather
 * than left to a spawn of the bare name, so a match is a real file and a refusal can name it.
 */
export function candidatePaths({ env = process.env, home = os.homedir(), platform = process.platform, io = fs } = {}) {
  ...
  const addDir = dir => {
    if (dir) for (const name of names) add(path.join(dir, name));
  };
  ...
  if (windows) {
    ...
    for (const release of releases) addDir(path.join(standalone, "releases", release));
  }

  const defaultInstallDir = windows
    ? env.LOCALAPPDATA && path.join(env.LOCALAPPDATA, "Programs", "Herdr", "bin")
    : path.join(home, ".local", "bin");
  addDir(env.HERDR_INSTALL_DIR || defaultInstallDir);

  // Windows spells the variable `Path`; `process.env` hides that, a plain object does not. An EMPTY
  // entry is skipped rather than joined, because joining it yields the bare name.
  const searchPath = String(env.PATH ?? env.Path ?? "");
  for (const dir of searchPath.split(windows ? ";" : ":")) addDir(dir);
  return found;
}
 ...
 *
 * FALLS BACK TO THE BARE NAME only when nothing was found, including on PATH, so the caller still
 * has something to spawn and the refusal carries every place that was searched.
 */
export function resolveHerdrBinary(options = {}) {
  ...
  return {
    ...
    tried: candidates,
    why:
      "no herdr binary was found where Herdr's installer puts it, and herdr is not on PATH. " +
      "Tried: " + (candidates.length ? candidates.join(", ") : "(nothing)") +
