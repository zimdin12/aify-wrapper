...
import { withoutAgentSession } from "./inherited-session.mjs";
...
export function herdrServerEnv(base, paths) {
  // AND NO AGENT SESSION. Every pane is a new terminal for a person, so nothing typed into one may inherit
  // the session `herdr-aify` happened to be run from. Measured 2026-09-15: a server started from inside
  // comms-tech-lead's Claude Code session handed that agent's id and conversation to every pane, and a bare
  // `claude-aify` in one started as comms-tech-lead and replaced the live one (lib/inherited-session.mjs).
  const env = withoutAgentSession(base);
