...
/**
 * Where codex files trust for a `-c` hook. The synthetic path is resolved against the filesystem ROOT, which
 * on Windows is the drive this process is running on -- read from the system rather than typed here, because
 * a host that keeps its profile on another drive would otherwise be given a key codex never writes.
 */
export function codexHookTrustKey(eventKey, { windows = process.platform === "win32", root = path.parse(process.cwd()).root } = {}) {
  const source = windows ? `${root.endsWith("\\") ? root : `${root}\\`}<session-flags>\\config.toml` : "/<session-flags>/config.toml";
