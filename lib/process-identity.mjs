 ...
 * else) or `unverified` (alive, and nothing to compare).
 *
...
export function identify(entry, { alive, startedAt }) {
  ...
  if (!startedAt) return "unverified";
...
/** `pid` and every parent above it that the table still lists. */
export function ancestors(pid, table) {
  const chain = new Set();
  for (let at = pid; Number.isInteger(at) && at > 0 && !chain.has(at); at = table?.get(at)?.ppid) chain.add(at);
  return chain;
}
 ...
 * itself is ended: an unreadable table never widens a kill.
 */
export function killTree(pid, { platform = process.platform, run = spawnSync, kill = process.kill, protect = {}, table, spare = [] } = {}) {
  ...
  const targets = [pid, ...descendants(pid, rows)].filter((target) => !isProtected(target, protect) && !spared.has(target));
  if (platform === "win32") {
    try {
