 ...
 */
export function parseLinuxStartedAt(statText, procStatText, bootEpoch = null) {
  const close = String(statText || "").lastIndexOf(")");
  if (close === -1) return null;
  const ticks = Number(String(statText).slice(close + 1).trim().split(/\s+/)[19]);
  const boot = bootEpoch ?? Number(/^btime\s+(\d+)/m.exec(String(procStatText || ""))?.[1]);
  if (!Number.isFinite(ticks) || ticks < 0 || !(boot > 0)) return null;
  return (boot + ticks / LINUX_USER_HZ) * 1000;
}

/**
 * The epoch second this boot began, FIXED for the boot, or null.
 *
 * `/proc/stat`'s `btime` is not fixed: the kernel derives it from the wall clock at each read, so a clock step
 * -- a VM resumed, WSL resynced after the host slept, a large time sync -- moves every start time computed
 * from it, and a live instance compared with its record by more than the tolerance read as a reused pid
 * (external review, 2026-09-15; not yet seen on a host). So the first reader after boot keeps what `btime`
 * said then, in a boot-scoped file named by this boot's id and this user, and every later reader computes
 * from that. A file this user does not own is not believed. Anything unreadable falls back to the live
 * `btime`, which is what every reader used before.
 */
export function linuxBootEpoch({ procStat, readFile = fs.readFileSync, writeFile = fs.writeFileSync, ownerOf = (file) => fs.statSync(file).uid, uid = process.getuid?.() } = {}) {
  const live = Number(/^btime\s+(\d+)/m.exec(String(procStat || ""))?.[1]);
  const fallback = live > 0 ? live : null;
  let bootId = "";
  try {
  ...
  if (!/^[0-9a-f-]{36}$/.test(bootId) || !Number.isInteger(uid)) return fallback;
  const anchor = `/dev/shm/aify-boot-${uid}-${bootId}`;
  const kept = () => {
    try {
      if (ownerOf(anchor) !== uid) return null;
      const value = Number(String(readFile(anchor, "utf8")).trim());
      return value > 0 ? value : null;
    } catch {
      return null;
    }
  };
  // Created only if absent (`wx`): the first reader of the boot wins, and every reader then reads what it kept.
  if (fallback) {
    try {
      writeFile(anchor, String(fallback), { flag: "wx", mode: 0o644 });
    } catch {
      // Already there, or /dev/shm is not writable: read what is there, or use the live value.
    }
  }
...
/**
 * How far this boot's anchored start times sit from the wall clock, in ms (0 on any platform without an
 * anchor).
 *
 * The anchor holds what `btime` said when the boot was first seen, so anchored start times agree with each
 * OTHER for as long as the host is up. They do NOT agree with `Date.now()`: the kernel re-derives `btime`
 * from the wall clock, so the two drift apart by exactly this offset -- measured on this machine's WSL on
 * 2026-09-16, the anchor was 121 s behind live `btime`, a day after it was written. Comparing an anchored
 * start time with a wall-clock moment is wrong by that much, which made a lock holder that had just taken
 * the lock read as a pid recycled after it (external review, 2026-09-16). Subtract this from an anchored
 * start time to put it on the wall clock.
 */
export function anchorOffsetMs({ platform = process.platform, readFile = fs.readFileSync, bootEpoch = linuxBootEpoch } = {}) {
  if (platform !== "linux") return 0;
  try {
    const procStat = String(readFile("/proc/stat", "utf8"));
    const live = Number(/^btime\s+(\d+)/m.exec(procStat)?.[1]);
    const anchored = bootEpoch({ procStat, readFile });
    if (!(live > 0) || !(anchored > 0)) return 0;
    return (anchored - live) * 1000;
  } catch {
    return 0;
  }
}
 ...
 */
export function startTimes(pids, { platform = process.platform, run = spawnSync, readFile = fs.readFileSync, strict = false, bootEpoch = linuxBootEpoch } = {}) {
  ...
  try {
    if (platform === "linux") {
      const procStat = String(readFile("/proc/stat", "utf8"));
      const boot = bootEpoch({ procStat, readFile });
      const found = new Map();
      for (const pid of wanted) {
        try {
          const at = parseLinuxStartedAt(String(readFile(`/proc/${pid}/stat`, "utf8")), procStat, boot);
 ...
 *
 * `offsetMs` is `anchorOffsetMs()`, and the caller passes it whenever the ENTRY's times came from a
 * wall clock -- a lock holder's moment, another program's record -- while `startedAt` is anchored. An
 * entry this code wrote is anchored on both sides and needs no offset.
 */
export function identify(entry, { alive, startedAt, offsetMs = 0 }) {
  if (!alive) return "gone";
  if (!startedAt) return "unknown";
  const onTheirClock = startedAt - offsetMs;
  const recorded = Number(entry?.startedAtMs);
  if (recorded) return Math.abs(onTheirClock - recorded) <= START_TIME_TOLERANCE_MS ? "ours" : "reused";
  const seen = Number(entry?.seenAliveAtMs);
  if (seen) return onTheirClock <= seen ? "ours" : "reused";
  return "unknown";
}
...
function parseLinuxParent(statText) {
  return linuxStatField(statText, 1);
}

/** The process group, field 5 of /proc/<pid>/stat. */
export function parseLinuxGroup(statText) {
  return linuxStatField(statText, 2);
}

function linuxStatField(statText, index) {
  const close = String(statText || "").lastIndexOf(")");
  const value = close === -1 ? NaN : Number(String(statText).slice(close + 1).trim().split(/\s+/)[index]);
 ...
 */
export function processTable({ platform = process.platform, run = spawnSync, readFile = fs.readFileSync, readDir = fs.readdirSync, bootEpoch = linuxBootEpoch } = {}) {
  try {
    if (platform === "linux") {
      const procStat = String(readFile("/proc/stat", "utf8"));
      const boot = bootEpoch({ procStat, readFile });
      ...
      for (const name of readDir("/proc")) {
        ...
        try {
          ...
          const startedAtMs = parseLinuxStartedAt(stat, procStat, boot);
          const ppid = parseLinuxParent(stat);
          // The group too: Linux re-parents a dead parent's children to init, and their group is what
          // still names the launcher that started them.
          if (startedAtMs && ppid !== null) rows.set(Number(name), { pid: Number(name), ppid, pgid: parseLinuxGroup(stat), startedAtMs });
        } catch {
...
/**
 * PURE. The processes that HOST another agent: every verified ancestor, in `table`, of a pid in `boundaries`
 * (another agent's leased processes). Ending one ends what runs inside it -- a Herdr server takes its panes
 * with it, an aify-env daemon its workers -- so none is ever part of this agent's tree to end.
 */
export function hostsOf(boundaries, table) {
  const hosts = new Set();
  if (!table) return hosts;
  for (const boundary of boundaries) {
    for (const above of ancestors(boundary, table)) if (above !== boundary) hosts.add(above);
  }
  return hosts;
}
 ...
 * `descendants`).
 *
 * NOTHING IS SIGNALLED AS A GROUP, and nothing that hosts another agent is ended (external review,
 * 2026-09-15). A POSIX group signal reached every member before `spare` or `boundaries` were consulted, so a
 * replace ended another agent's launcher that shared the group. And a daemon started from inside the session
 * -- a Herdr server, an aify-env -- was part of the tree, so a replace ended every agent in its panes. Group
 * members, which is how a runtime re-parented to init is still found, are now signalled one by one under the
 * same exclusions as the tree; a `pid` that hosts another agent is not ended at all (the caller refuses).
 */
/**
 * The processes a GONE parent left running: rows naming `entry.pid` as their parent -- or, where the table
 * carries groups, as their process group -- that started within its life, never below `boundaries`. Their
 * own trees are for `killTree`.
 *
 * Windows never clears a dead parent's pid from its children. Linux does: it re-parents them to init, and
 * what still names the launcher is the group it led (measured under WSL 2026-09-15, where a killed
 * launcher's runtime was found by neither parent nor tree). The life bounds both: a process that took the
 * pid afterwards names it for its own children too. An entry with nothing to date its start by has no
 * findable children.
 */
export function orphanedChildren(entry, table, { boundaries = new Set() } = {}) {
  const since = entry?.startedAtMs ?? entry?.seenAliveAtMs;
  if (!table || !Number.isInteger(entry?.pid) || !Number.isFinite(since)) return [];
  const successor = table.get(entry.pid);
  const until = successor ? successor.startedAtMs : Infinity;
  return [...table.values()].filter((row) => (row.ppid === entry.pid || row.pgid === entry.pid) && row.pid !== entry.pid && !boundaries.has(row.pid)
    && Number.isFinite(row.startedAtMs) && row.startedAtMs >= since && row.startedAtMs < until);
}
...
export function killTree(pid, { platform = process.platform, run = spawnSync, kill = process.kill, protect = {}, table, spare = [], boundaries = [] } = {}) {
  ...
  const others = new Set(boundaries);
  const standing = new Set([...others, ...hostsOf(others, rows)]);
  if (spared.has(pid) || standing.has(pid)) return false;
  const tree = [pid, ...descendants(pid, rows, standing)];
  // A member is left when it hosts another agent or runs beneath one. Merely sharing an ancestor with one is not
  // hosting it: init is above every re-parented runtime, which is exactly what a group member usually is.
  const group = platform === "win32" || !rows ? [] : [...rows.values()]
    .filter((row) => row.pgid === pid && !tree.includes(row.pid) && !standing.has(row.pid) && ![...ancestors(row.pid, rows)].some((above) => others.has(above)))
    .map((row) => row.pid);
  const targets = [...tree, ...group].filter((target) => !isProtected(target, protect) && !spared.has(target));
  if (platform === "win32") {
    try {
