...
export class HerdrAifyInstance {
  ...
  async stop({ env = {}, herdrBin = "herdr" } = {}) {
    if (this.#stopped) return Object.freeze({ stopped: true, already: true, everServed: this.#serving, serverStopped: false, killed: false, confirmedGone: true });
    ...
    if (!this.#serving) {
      this.#stopped = true;
      return Object.freeze({ stopped: true, already: false, everServed: false, serverStopped: false, killed: false, confirmedGone: true });
    }
    this.#stopped = true;
