...
import { resolveOnPath } from "./path-lookup.mjs";
...
export class HerdrAifyInstance {
   ...
   * @param lookup     resolves a command name on a given PATH, injected so no test depends on what
   *                   happens to be installed on the machine running it.
   */
  constructor({ profileRoot, invocation, platform = process.platform, processes, clock, lookup = resolveOnPath }) {
    this.#lookup = lookup;
  ...
  async start({ env = {}, herdrBin = "herdr", envBin = "aify-env", io = fs, withEnv = true, pluginDir = null } = {}) {
    ...
    // THE BINARY IS LOOKED FOR BEFORE THE PANE IS TOLD TO RUN IT, on the PATH the pane will use.
    // `pane run` types a command into a shell and answers ok whether or not that command exists, so
    // an `aify-env` that is not installed, or whose global package link points at a directory this
    // host no longer has, was indistinguishable from a healthy start until the receipt failed to
    // arrive thirty seconds later — and the message then had to offer three reasons because nothing
    // had told them apart. MEASURED 2026-09-16: that is exactly how a host looked after a restart
    // moved the directory its global link named.
    const onPath = this.#lookup(envBin, { env: serverEnv, platform: this.#platform, io });
    if (!onPath.ok) return this.#failed("env", onPath.why);
