...
/**
 * The longest path a Unix socket can bind: `sun_path` holds 108 bytes on Linux and 104 on macOS and the
 * BSDs, one of them the terminating NUL.
 *
 * MEASURED ON LINUX, 2026-09-16: an owner endpoint of 119 bytes did not fail as too long. The kernel
 * bound a TRUNCATED path, and `listen` reported `EADDRINUSE` for a socket nobody held -- so a launch
 * under a deep profile root died on an error naming the wrong cause.
 */
export function unixSocketPathMax(platform = process.platform) {
...
export function instancePaths({ profileRoot, invocation, platform = process.platform }) {
  ...
  if (platform !== "win32") {
    const longest = Buffer.byteLength(path.join(root, "herdr.sock"));
    if (longest > unixSocketPathMax(platform)) {
      throw new Error(
        `herdr_instance: the profile root is too deep for a socket (${longest} bytes, ${platform} allows ${unixSocketPathMax(platform)}); `
          + "use a shorter profile root",
      );
    }
  }
